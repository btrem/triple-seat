	...
	script.onload = function() {
		...
		var TripleSeat = {
			...
			HtmlUtils : {
				...
				outputPartyDuration: function() {
					...
					var duration = TripleSeat.Utils.calculateTimeDifference(startTime, endTime);

					// show duration in form next to inputs for start and end time
					$('output[for~="start_time"][for~="end_time"]').val(duration);
				...
				showFormErrors: function(formErrors) {
					...
					$.each(formErrors, function(field, messages) {
						if (field == 'Base') { // silly rails default field
							field = '';
						}
						$.each(messages, function(i, error) {
							$('<li />').text(field + ' ' + error).appendTo(errorList);
						});
					});

					$('#triple-seat-container').addClass('tripleSeatError');

					$('#triple-seat-container > .formSubmissionInfo').empty().append('<h1>Some information is missing or has errors</h1><p>Please fix the following:</p>').append(errorList);
				...
				showSuccessMsg: function(msg) {
					...
					$('#triple-seat-container > .formSubmissionInfo').empty().append('<h1>Request received</h1><p>' + msg + '</p>');
				...
				scrollToMessage: function() {
					$('html, body').animate({
					  scrollTop: $('#triple-seat-container > .formSubmissionInfo').offset().top
					}, 200);
				},
		...
		$(document).ready(function(){
			...
			var Config = {
				...
				StartTime : {
					min : function(){
							if (typeof window.start_time_min !== 'undefined' && TripleSeat.Utils.validateTime(window.start_time_min)) {
								return TripleSeat.Utils.roundTime(window.start_time_min);
							}
							return "00:00";
						}(),

					max : function(){
							if (typeof window.start_time_max !== 'undefined' && TripleSeat.Utils.validateTime(window.start_time_max)) {
								return TripleSeat.Utils.roundTime(window.start_time_max);
							}
							return "23:45";
						}(),

					val : function(){
							if (typeof window.start_time_default !== 'undefined' && TripleSeat.Utils.validateTime(window.start_time_default)) {
				...
				EndTime : {
					min : function(){
							if (typeof window.end_time_min !== 'undefined' && TripleSeat.Utils.parseTime(window.end_time_min)) {
					...
					max : function(){
							if (typeof window.end_time_max !== 'undefined' && TripleSeat.Utils.parseTime(window.end_time_max)) {
					...
					val : function(){
							if (typeof window.end_time_default !== 'undefined' && TripleSeat.Utils.parseTime(window.end_time_default)) {
				...
				EventDate : {
					min : function(){
							if (typeof window.date_min !== 'undefined' && Number.isInteger(window.date_min) && window.date_min >= 0) {
								var targetDate = new Date();
								targetDate.setDate(targetDate.getDate() + window.date_min);
								return targetDate;
							}
							return new Date;
						}(),
					...
					max : function(){
							var dateAdjustment;
							if (typeof window.date_max !== 'undefined' && Number.isInteger(window.date_max) && window.date_max >= 0) {
					...
					val : function(){
							if (typeof window.date_default !== 'undefined' && Number.isInteger(window.date_default)) {
								var targetDate = new Date();
								targetDate.setDate(targetDate.getDate() + window.date_default);
								return targetDate;
							}
							return new Date;
						}()
				...
				GuestCount : {
					min : function(){
							if (typeof window.guest_count_min !== 'undefined' && Number.isInteger(window.guest_count_min) && window.guest_count_min > 0) {
								return window.guest_count_min;
							}
							return 1;
						}(),
					max : function(){
							if (typeof window.guest_count_max !== 'undefined' && Number.isInteger(window.guest_count_max)) {
								return window.guest_count_max;
							}
							return 1000;
						}(),
					val : function(){
							if (typeof window.guest_count_default !== 'undefined' && Number.isInteger(window.guest_count_default)) {
			...
			if (Config.StartTime.val < Config.StartTime.min || Config.StartTime.val > Config.StartTime.max) {
			...
			if (Config.EndTime.val < Config.EndTime.min || Config.EndTime.val > Config.EndTime.max) {
			...
			if (Config.EventDate.val < Config.EventDate.min || Config.EventDate.val > Config.EventDate.max) {
			...
			if (Config.GuestCount.val < Config.GuestCount.min || Config.GuestCount.val > Config.GuestCount.max) {
			...
			var elementFontSize, tripleSeatQueryString, queryStringName, queryStringValue;

			tripleSeatQueryString = TripleSeat.HtmlUtils.getQueryString();

			$('form.tripleSeat').attr({
				action: 'https://api.tripleseat.com/v1/leads/create.js?' + tripleSeatQueryString.replace(/&/g, '&amp;')
			});
			...
			for (var i=0, j=tripleSeatQueryString.length; i<j; i++) {
				...
				queryStringName = tripleSeatQueryString[i].slice(0, tripleSeatQueryString[i].indexOf('='));
				queryStringValue = tripleSeatQueryString[i].slice(tripleSeatQueryString[i].indexOf('=') + 1);
			...
			if (Html5Feature.TimeInput) {
				...
				$('.startTimeFieldContainer').append('<input id="start_time" name="lead[start_time]" required type="time" step="900">');
				$('.endTimeFieldContainer').append('<input id="end_time" name="lead[end_time]" required type="time" step="900">');

				// set min and max attributes for start time and end time input elements
				if (Config.StartTime.min !== "00:00") {
					$('.startTimeFieldContainer > input[type="time"]').attr('min', Config.StartTime.min);
				}
				if (Config.StartTime.max !== "23:45") {
					$('.startTimeFieldContainer > input[type="time"]').attr('max', Config.StartTime.max);
				}

				if (Config.EndTime.min !== "00:00") {
					$('.endTimeFieldContainer > input[type="time"]').attr('min', Config.EndTime.min);
				}
				if (Config.EndTime.max !== "23:45") {
					$('.endTimeFieldContainer > input[type="time"]').attr('max', Config.EndTime.max);
				}
				...
				$('.startTimeFieldContainer > input[type="time"], .endTimeFieldContainer > input[type="time"]').blur(function(){

					if (this.validity.stepMismatch && !this.validity.rangeUnderflow && !this.validity.rangeOverflow) {
				...
				var selectElementStartTime = $('<select id="start_time" name="lead[start_time]" />');
				var selectElementEndTime = $('<select id="end_time" name="lead[end_time]" />');

				var hours, minutes, hoursPadded, minutesPadded, hours12Formatted, ampm, formattedTime;
				...
				for (hours = 0; hours < 24; hours++) {
					...
					for (minutes = 0; minutes <= 45; minutes += 15) {
						...
						if (formattedTime >= Config.StartTime.min && formattedTime <= Config.StartTime.max) {
							// create <option> elements and append them to <select> variable
							$('<option />').attr({
								value: hoursPadded + ":" + minutesPadded
							}).text(hours12Formatted + ":" + minutesPadded + ampm).appendTo(selectElementStartTime);

						}

						if (formattedTime >= Config.EndTime.min && formattedTime <= Config.EndTime.max) {
							// create <option> elements and append them to <select> variable
							$('<option />').attr({
								value: hoursPadded + ":" + minutesPadded
							}).text(hours12Formatted + ":" + minutesPadded + ampm).appendTo(selectElementEndTime);
			...
			$('[name="lead[start_time]"], [name="lead[end_time]"]').change(function(){
				TripleSeat.HtmlUtils.outputPartyDuration();
			});
			...
			$('[name="lead[start_time]"], [name="lead[end_time]"]').focus(function(){
				$('output[for~="start_time"][for~="end_time"]').animate({opacity: 1}, 100);
			});
			...
			$('[name="lead[start_time]"], [name="lead[end_time]"]').blur(function(){
				window.setTimeout(function(){
					if (!$('[name="lead[start_time]"]').is(":focus") && !$('[name="lead[end_time]"]').is(":focus")) {
						$('output[for~="start_time"][for~="end_time"]').animate({opacity: 0}, 800);
					}
				});
			});
			...
			if (Config.GuestCount.val) {
				$('input[type="number"][name="lead[guest_count]"]').val(Config.GuestCount.val);
			}
			...
			if (Html5Feature.DateInput) {
				...
				$('[name="lead[event_date]"]').attr({
					min: TripleSeat.Utils.dateObjectToString(Config.EventDate.min),
					max: TripleSeat.Utils.dateObjectToString(Config.EventDate.max)
				});

				if (Config.EventDate.val) {
					$('[name="lead[event_date]"]').val(TripleSeat.Utils.dateObjectToString(Config.EventDate.val));
				}

				$('[name="lead[event_date]"]').change(function(){
					if ((this).validity.rangeUnderflow) {
						this.setCustomValidity('The event date you have chosen is too early.');
					}
					else if ((this).validity.rangeOverflow) {
						this.setCustomValidity('The event date you have chosen is too far in the future.');
					}
					else {
				...
				$.ajax({
					...
					success: function(){
						...
						$('[name="lead[event_date]"]').removeAttr('name').parent().after('<input type="hidden" name="lead[event_date]">');

						// finally, add datepicker
						$('.tripleSeat input[type="date"]').datepicker({
				...
				$.ajax({
					...
					success: function(data) {
						if (data.errors != undefined) {
							...
							TripleSeat.HtmlUtils.showSuccessMsg(data.success_message);
						}
					},
					complete: function() {
